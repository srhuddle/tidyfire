...
## County summary example

`tidyfire` can also roll tract-level records up into county summaries. This example
pulls yearly `raw` and `estimated` totals for Adams County, Pennsylvania using
the county summary endpoint, then prints two simple console tables.

- `state_geoid = "42"` is Pennsylvania
- `county_geoid = "42001"` is Adams County, Pennsylvania
- county summary responses currently return `countyGEOID`, so the display name is
  supplied directly in the example
